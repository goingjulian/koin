...
:::info 
Koin doesn't have any import concept. Koin definitions are lazy: a Koin definition is started with the Koin container but is not instantiated. An instance is created only when a request for its type has been done.
:::
